...
| — | **From Expert Systems to LLM Agents** (knowledge-based systems, rule-based inference, expert systems, fuzzy logic, stateful/ReAct/proactive agents — history/foundation piece for the RAG series) | [`from-expert-systems-to-agents/`](from-expert-systems-to-agents/) | Draft started (2026-09-16) — article + short post + 3 figures (titel, knowledge_systems_timeline, fuzzy_logic_membership) done; PNGs exported (titel, timeline, 2x); fuzzy_logic_membership.png export pending (one chromium command, sandbox approval needed in agent context). Source image: symbolics_3640.jpg (Wikimedia CC BY-SA — verify on file page). Open: export fuzzy PNG, verify links live, publish checklist; referenced from ontology-driven-rag as foundation article. |
