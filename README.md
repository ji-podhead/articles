...
| — | **You Have Logs. You Don't Have a SIEM.** (generic SMB guide: AWS-native vs. open source, Suricata/eBPF/firewall/WAF, 4 paths: OSS/AWS/SaaS/minimal) | [`siem-smb-guide/`](siem-smb-guide/) | Draft done, expanded 2026-09-10 — article (~6300 words) + short post + 7 SVG figures + 1 real Wazuh dashboard screenshot (`wazuh-dashboard-incident-response.png`, third-party source, attributed) + PNGs, link placeholders open. Additionally covers: Datadog/Splunk as the third (SaaS) path incl. per-service deploy + egress cost reality, GCP Cloud Logging/Ops-Agent costs, Kafka-vs-Redis-Streams as a transport-layer question, Elastic-Stack license history (SSPL vs. OpenSearch fork), end-to-end AI threat-detection wiring for AWS (SageMaker→Lambda→Security Hub, Bedrock Agents) and on-prem (OpenSearch ML Commons). All ~35 external links verified live (200 + content spot-check, 2026-09-10). Open: 2 more SVG tables (Cloudflare product overview, alternatives comparison) waiting on logo assets. **Deliberately without JiMesh reference** (operator directive 2026-09-09: generic until after release) — hence **without** the otherwise mandatory mesh-cube brand in the title figure. The JiMesh connection follows as its own follow-up/article after release. Complements (does not replace) the existing JiMesh-specific SIEM article under [`siem/`](siem/). Context: also serves the operator as interview prep (Head of MLOps at CloudiQS, 2026-09-11). |
