...
**Shodan** is the original and still the reference point — strong for IoT, industrial control systems, and anything with a distinctive banner. **Censys** overlaps heavily but is the stronger tool specifically for certificate-based infrastructure mapping: give it a company's TLS certificate pattern and it finds every server sharing it, regardless of IP range.
...
**Ollama** listens on **11434** and answers a plain request with the literal string `"Ollama is running"` — searchable directly (`port="11434" && body="Ollama is running"` on FOFA, `port:11434 "Ollama is running"` on Shodan). **vLLM**'s OpenAI-compatible server defaults to **8000**, exposing `/v1/models` and `/v1/chat/completions`. **Qdrant** exposes its HTTP API on **6333** and gRPC on **6334**, identifiable by a JSON banner containing `"qdrant - vector search engine"`; **Milvus** uses gRPC on **19530**; **ChromaDB** defaults to **8000** and returns version metadata (`nanoseconds_since_epoch`) on an unauthenticated root request. **Gradio** (the default UI for most Hugging Face demos and Stable Diffusion frontends) runs on **7860**; **Streamlit** dashboards on **8501**. **Jupyter Notebook/Lab** sits on **8888**.
...
An exposed instance in this category is rarely "just" a data leak: an open Qdrant or Milvus hands over your actual embedded knowledge base in re-searchable form; an open Ollama or vLLM is free inference on your GPU bill; an exposed Jupyter with its token disabled is arbitrary code execution on the host, not just a data problem. And the same exposure logic applies to a team's own local dev tooling — a CLI agent that doesn't take a `-p` flag on its command line isn't network-invisible if something else in its stack (a browser control panel, a webhook receiver, an IPC companion process) starts a local HTTP server that ends up bound to `0.0.0.0` instead of `127.0.0.1`.
