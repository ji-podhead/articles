...
Any instance is identifiable by a Swagger UI titled "LiteLLM API" at `/` and `/docs` — `http.title:"LiteLLM API"` is the reasonable starting query on Shodan (no dedicated product fingerprint exists, and this hasn't been confirmed against live results, so treat it as a starting point). One correction worth making explicitly, caught during fact-checking this piece rather than before publishing: `/health` is **not** a usable open-vs-protected signal — it's hardcoded behind the same auth dependency as everything else regardless of whether a master key is configured, so it returns 401 either way. The one endpoint that genuinely skips auth unconditionally, `/health/liveliness`, is a plain Kubernetes liveness probe that returns the same `"I'm alive!"` on *every* instance — it tells you the process is running, nothing about its auth configuration. We could not confirm a reliable passive signal that distinguishes a keyless instance from a protected one without sending an authenticated-looking request to an actual inference endpoint, which this series deliberately doesn't do against instances that aren't your own. Worst case for a genuinely unauthenticated instance isn't just inference theft on someone else's bill: with no master key set, the admin surface — key creation, spend tracking, model configuration — is reachable too, which is a wider compromise than the proxy's own "virtual key" model was designed to allow. Separately, and independently confirmed against NVD, **CVE-2024-6825** (CVSS 8.8) was a real remote-code-execution bug in LiteLLM's `post_call_rules` config handling: a callback value gets split at its final `.` and the remainder imported as a Python module, so setting the callback to something like `os.system` let an attacker run arbitrary commands whenever a chat response was processed — fixed in v1.65.4.dev6. That one required some existing configuration access to trigger, which is exactly why leaving the master key unset in the first place is the finding that actually matters here.
...
- LiteLLM PR #11378 (liveliness route auth removal) — https://github.com/BerriAI/litellm/pull/11378
- LiteLLM issue #9576 (/health 401 behavior) — https://github.com/BerriAI/litellm/issues/9576
