...
WPT=$(LC_ALL=C awk "BEGIN{printf \"%.2f\", $W*0.75}")
HPT=$(LC_ALL=C awk "BEGIN{printf \"%.2f\", $H*0.75}")
PRINT="$D/.$NAME.print.html"
...
open(sys.argv[4], 'w').write(t)
PYEOF
UP="file://$PRINT"
PDF="$(cd "$ART" && pwd)/.$NAME.pdf"
chromium --headless=new --disable-gpu --no-sandbox --print-to-pdf="$PDF" --no-pdf-header-footer "$UP" >/dev/null 2>&1
if [ -f "$PDF" ]; then
  PAGES=$(pdfinfo "$PDF" 2>/dev/null | awk '/^Pages:/ {print $2}')
  [ "$PAGES" = "1" ] || echo "WARN: PDF hat $PAGES Seiten - Figur ist zu hoch fuer H=$H (SVG verliert Inhalt)"
