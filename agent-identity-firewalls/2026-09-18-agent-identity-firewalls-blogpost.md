...
![CVE-2025-47241 domain-allowlist bypass in Browser Use, with real mitigation effectiveness data](browser_security.svg)

*Figure 1: the CVE, and which mitigation actually works. Planner/executor isolation is the only one that reaches 0%.*
...
![The four-layer agent firewall stack: ingress identity, tool-call policy, egress and data, audit and receipts](agent_firewall_stack.svg)

*Figure 2: identity answers who, policy answers what, egress answers where, audit answers what happened.*
...
![Four-step gateway flow: strip identity headers, resolve server-side, re-stamp, deterministic policy](identity_chain.svg)

*Figure 3: strip, resolve, re-stamp, decide. The client never gets a vote on its own identity.*
...
![Meta, OpenAI, Google, AWS approaches converging on per-agent credentials, server-side identity resolution, deterministic policy](big_tech_approaches.svg)

*Figure 4: four teams converged on the same shape independently. That's a signal, not a coincidence.*
...
Browser Use's CVE is the app-level version of the same mistake: an allowlist trusting the agent's own URL parsing. The data side closes the exfiltration channel from Figure 1: **taint tracking** blocks outbound calls after a credential read (AgentWall ships this statefully, killing read-then-exfiltrate chains instead of matching them), and **response scanning** redacts secrets and PII before they reach the model's context (Agent Wall: 14 secret patterns, 5 PII patterns, pass/redact/block).
...
![The verified open-source landscape: Pipelock, Agent Wall, LlamaFirewall, Cedar/OPA, SPIFFE, Coder across the layers](frameworks_landscape.svg)

*Figure 5: every entry verified against a live primary source.*

![Agent Wall dashboard live capture](agentwall.png)

*Figure 6: Agent Wall run live against real injection payloads: 12 calls scanned, 6 denied, 4 attacks caught, patterns named per event.*
...
![A per-agent scoping field written correctly, read never](honest_gap.svg)
